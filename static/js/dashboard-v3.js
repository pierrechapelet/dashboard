...
//console.log(directoratesJson);
  ...
  //var totalstudentsDim = ndx.dimension(function(d) { return d["NbStudents"]; });
  ...
  // List of fields to sum up
  var fields = ['NbMaleStudents', 'NbFemaleStudents']; // whatever fields you need
  // map reduce by a dimension to get the number:
  var NbStudByOwership = ndx.groupAll().reduce(reduceFieldsAdd(fields), reduceFieldsRemove(fields), reduceFieldsInitial(fields)); 
  console.log(NbStudByOwership);
  testfunction(NbStudByOwership);


  //var totalstudents = ndx.groupAll().reduceSum(function(d) { return d["NbStudents"];});
  ...
  //test
  var totalsumstudChart = dc.numberDisplay("#totalsum-stud-nb");
  var studentChart = dc.barChart("#totalsum-stud-nb");
  ...
  //    console.log(d.NbMaleStudents);
  //    return d; })
  .margins({top: 10, right: 50, bottom: 30, left: 50})
  .centerBar(false)
  .gap(5)
  .elasticY(true)
  .x(d3.scale.ordinal().domain(numSchoolsByOwnership))
  ...
  totalsumstudChart
    ...
    .group(NbStudByOwership)
    .on("postRender", function(chart, filter){
      //testfunction(chart);
      //console.log(NbStudByOwership);
      //console.log(filter);
    });
  ...
  function testfunction(d) {
      ...
      console.log(NbStudByOwership.data)
      console.log(d)
  }
